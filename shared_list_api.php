...
switch ($action) {
    ...
    case 'category_add':
        if (trim($p['name'] ?? '') !== '') {
            $list['categories'][] = ['id' => 'cat' . uniqid(), 'name' => trim($p['name']), 'items' => []];
        }
        break;

    case 'category_delete':
        $list['categories'] = array_values(array_filter($list['categories'], fn($c) => $c['id'] !== ($p['id'] ?? '')));
        break;

    case 'category_rename':
        $cat = &shared_cat_find($list, $p['id'] ?? '');
        if ($cat && trim($p['name'] ?? '') !== '') $cat['name'] = trim($p['name']);
        unset($cat);
        break;

    case 'item_add':
        $cat = &shared_cat_find($list, $p['categoryId'] ?? '');
        if ($cat && trim($p['title'] ?? '') !== '') {
